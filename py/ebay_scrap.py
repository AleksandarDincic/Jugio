...
url = 'https://www.ebay.com/sch/i.html?_from=R40&_nkw=yu-gi-oh&_sacat=2536&_pgn='
headers = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:101.0) Gecko/20100101 Firefox/101.0'}

def scrapeEbayImages(startPage, endPage):
    ...
    for i in range(startPage, endPage + 1):
        ...
        if req.status_code == 200:
            ...
            imgs = soup.find_all('img', {'class': 's-item__image-img'})
            cnt = len(imgs) * (i-1)
            for img in imgs:
                imgLink = img.get('src').replace('s-l225', 's-l600')
                imgName = 'img_' + str(cnt) + '.jpg'
                cnt = cnt + 1
